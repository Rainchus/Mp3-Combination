...
# Get all .c files in the src/ directory and subdirectories
c_files = glob.glob('src/**/*.c', recursive=True)

# Get all .s files in the asm/ directory and subdirectories
s_files = glob.glob('asm/**/*.s', recursive=True)

# .asm files are assembly files hardcoded into the build system
asm_files = glob.glob('asm/**/*.asm', recursive=True)

print("C files found:", c_files)
print("S files found:", s_files)
print("ASM files found:", asm_files)
...
with open('build.ninja', 'w') as buildfile:
    ...
    ninja.rule(
        "n64crc",
        command="n64crc.exe rom/mp3-mp2.mod.z64",
        description="Running n64crc.exe on mod.z64"
    )
